...
class NutritionController(BaseController):
    ...
    def meal_index(cls):
        ...
        if user and user.get('weight_kg') and user.get('height_cm') and user.get('age'):
            ...
            calorie_goal = int(calculate_tdee(
                bmr, float(user.get('activity_level', 1.55))))
    ...
    def add_meal(cls):
        ...
        MealModel.add(uid, name, meal_type, calories,
        ...
        if saved and saved.get('calorie_goal'):
            ...
            if user and user.get('weight_kg') and user.get('height_cm') and user.get('age'):
                ...
                if wkg and hcm and age:
                    bmr = calculate_bmr(
                        wkg, hcm, age, user.get('gender', 'male'))
                    calorie_goal = int(calculate_tdee(
                        bmr, float(user.get('activity_level') or 1.55)))
        CalorieModel.log(uid, log_date, int(
            totals.get('cal', 0) or 0), calorie_goal)
        ...
        if saved and saved.get('calorie_goal'):
            ...
            if user and user.get('weight_kg') and user.get('height_cm') and user.get('age'):
                ...
                if wkg and hcm and age:
                    ...
                    calorie_goal = int(calculate_tdee(
                        bmr, float(user.get('activity_level') or 1.55)))
        CalorieModel.log(uid, today, int(
            totals.get('cal', 0) or 0), calorie_goal)
    ...
    def bmi_index(cls):
        ...
        if latest:
            ...
            if prefill_weight and prefill_height:
                bmi, bmi_label, bmi_color = calculate_bmi(
                    float(prefill_weight), float(prefill_height))
    ...
    def _build_report_data(cls, uid, days=7):
        ...
        for i in range(days - 1, -1, -1):
            ...
            if has_sleep:
                try:
                    ...
                    sleep_data.append(
                        round(float(sr['hours_slept']) if sr and sr.get('hours_slept') else 0, 1))
                except Exception:
                    sleep_data.append(0)
            else:
                sleep_data.append(0)
            ...
            if has_mood:
                try:
                    ...
                    mood_data.append(
                        int(mr['energy_level']) if mr and mr.get('energy_level') else 0)
                except Exception:
                    mood_data.append(0)
            else:
                mood_data.append(0)
            ...
            if has_weight:
                try:
                    ...
                    weight_data.append(
                        float(wr['weight_kg']) if wr and wr.get('weight_kg') else 0)
                except Exception:
                    weight_data.append(0)
            else:
                weight_data.append(0)

        avg_cal = round(sum(cal_data) / days, 1)
        total_cal = sum(cal_data)
        avg_prot = round(sum(prot_data) / days, 1)
        avg_carbs = round(sum(carbs_data) / days, 1)
        avg_fat = round(sum(fat_data) / days, 1)
        ...
        avg_sleep = round(sum(sleep_data) / days, 1)
        avg_mood = round(sum([m for m in mood_data if m > 0]) /
                         max(len([m for m in mood_data if m > 0]), 1), 1)
    ...
    def _generate_pdf(cls, report_type, days):
        ...
        health_risks = WellnessController._compute_risks(
            user, data['current_bmi'], avg_cal_7d)

        workout_plan_data = WellnessController._build_workout_plan_data(
            uid, user)
        ...
        try:
            ...
            DARK = rl_colors.HexColor('#0f1629')
            DARK2 = rl_colors.HexColor('#141e32')
            WHITE = rl_colors.white
            BORDER = rl_colors.HexColor('#1e2d50')
            MUTED = rl_colors.HexColor('#94a3b8')

            title_s = ParagraphStyle('T', parent=styles['Normal'],
                                     fontSize=22, textColor=TEAL, fontName='Helvetica-Bold', spaceAfter=4)
            ...
            body_s = ParagraphStyle('B', parent=styles['Normal'],
                                    fontSize=9, textColor=WHITE, leading=14, spaceAfter=4)
            rec_s = ParagraphStyle('R', parent=styles['Normal'],
                                   fontSize=9, textColor=WHITE, leading=14, leftIndent=10, spaceAfter=3)
            ...
            # ── Cover ──
            story.append(
                Paragraph(f'VitaPulse — {report_type} Health Report', title_s))
            ...
            if has_sleep_data or has_mood_data:
                story.append(Paragraph('😴 Sleep & Mood Log', h2_s))
                sm_cols = ['Date', 'Sleep (hrs)', 'Mood Score']
                sm_rows = [sm_cols]
                for i, label in enumerate(data['labels']):
                    sm_rows.append([
                        label,
                        str(data['sleep_data'][i]
                            ) if data['sleep_data'][i] > 0 else '—',
                        str(data['mood_data'][i]
                            ) if data['mood_data'][i] > 0 else '—',
                    ])
            ...
            if bmi_hist:
                story.append(Paragraph('⚖️ BMI History', h2_s))
                bmi_cols = [
            ...
            story.append(Paragraph('⚠️ Health Risk Assessment', h2_s))
            story.append(Paragraph(
            ...
            for key, part in workout_plan_data['body_parts'].items():
                wp_rows.append(
                    [part['name'], str(len(part['exercises'])), f"~{part['total_kcal']} kcal"])
            wp_rows.append(['Cardio Options', str(
                len(workout_plan_data['cardio'])), f"~{workout_plan_data['cardio_total']} kcal"])
            ...
            if active_meds:
                ...
                for m in active_meds:
                    med_rows.append(
                        [m['name'], m.get('dosage') or '—', m['times'], m['frequency']])
            ...
            if expense_by_cat:
                story.append(
                    Paragraph('💰 Health Expense Summary (This Month)', h2_s))
                story.append(Paragraph(
                ...
                for ec in expense_by_cat:
                    exp_rows.append(
                        [ec['category'], str(ec['total']), str(ec['cnt'])])
            ...
            story.append(
                Paragraph('Based on your logged health data:', body_s))
            ...
            if recs.get('workout'):
                ...
                for r in recs['workout']:
                    story.append(Paragraph(f'• {r}', rec_s))
            if recs.get('weight'):
                story.append(Paragraph('⚖️ Weight & BMI', h3_s))
            ...
            story.append(Paragraph(
                'Generated by VitaPulse · vitapulse.app · Data is for informational purposes only.',
    ...
    def _build_recommendations(cls, uid, user, data=None):
        ...
        if avg_cal > 0:
            ...
            recs['nutrition'].append(
                'Start logging your meals to get personalised calorie recommendations.')

        # Protein recommendations
        ideal_protein = round(
            float(user.get('weight_kg') or 70) * 1.6, 1) if user else 112.0
        if avg_prot > 0:
        ...
        if avg_sleep > 0:
            ...
            recs['sleep'].append(
                'Log your sleep in the Sleep Tracker to receive personalised sleep recommendations.')
        ...
        if cur_bmi > 0:
            ...
            recs['weight'].append(
                'Log your weight and BMI to receive personalised weight management tips.')
        ...
        if user and user.get('weight_kg') and user.get('height_cm') and user.get('age'):
            try:
                ...
                macros = calculate_macros(
                    calorie_goal, user.get('fitness_goal', 'maintain'))
    ...
    def notifications_mark_read(cls):
        uid = cls.uid()
        nid = request.json.get(
            'id') if request.is_json else request.form.get('id')
        if nid == 'all':
